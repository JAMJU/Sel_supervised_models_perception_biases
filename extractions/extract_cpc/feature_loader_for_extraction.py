...
from data_loader import parseSeqLabels
...
def getEncoder(args):
    ...
    if args.encoder_type == 'mfcc':
        from model_for_extraction import MFCCEncoder
        return MFCCEncoder(args.hiddenEncoder)
    elif args.encoder_type == 'lfb':
        from model_for_extraction import LFBEnconder
        return LFBEnconder(args.hiddenEncoder)
    else:
        from model_for_extraction import CPCEncoder
...
def getAR(args):
    if args.arMode == 'transformer':
        ...
        from model_for_extraction import NoAr
        arNet = NoAr()
    else:
        from model_for_extraction import CPCAR
...
def loadModel(pathCheckpoints, device, loadStateDict=True):
    models = []
    ...
    return ConcatenatedModel(models).to_device(device), hiddenGar, hiddenEncoder
...
def buildIntermediateFeature(featureMaker, layer, seqPath, strict=False,
                 maxSizeSeq=64000, seqNorm=False):
    r"""
    Apply the featureMaker to the given file.
    Arguments:
    ...
    while start < sizeSeq:
        ...
        with torch.no_grad():
            outi, features, _ = featureMaker.intermediate_forward(subseq, None, layer)
            if layer == 'AR':
                features = outi
            if seqNorm:
                features = seqNormalization(features)
