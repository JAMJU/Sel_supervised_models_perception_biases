...
class CPCEncoder(nn.Module):
    ...
    def intermediate_forward(self, x, layer='conv5'):
...
class CPCModel(nn.Module):
    ...
    def intermediate_forward(self, batchData, label, layer):
        ...
        if layer in ['conv1', 'conv2', 'conv3', 'conv4', 'conv5']:
            encodedData = self.gEncoder.intermediate_forward(batchData, layer)
        else:
            encodedData = self.gEncoder.intermediate_forward(batchData, layer)
            encodedData = encodedData.permute(0, 2, 1)
            cFeature = self.gAR(encodedData)
            print("AR output shape: {}".format(cFeature.shape))
        return cFeature, encodedData, label
...
class ConcatenatedModel(nn.Module):
    ...
    def intermediate_forward(self, batchData, label, layer):
        ...
        outEncoded = []
        for model in self.models:
            cFeature, encodedData, label = model.intermediate_forward(batchData, label, layer)
            outFeatures.append(cFeature)
            outEncoded.append(encodedData)
        return torch.cat(outFeatures, dim=2), \
               torch.cat(outEncoded, dim=2), label
