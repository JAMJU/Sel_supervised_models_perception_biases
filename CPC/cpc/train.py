...
def run(trainDataset,
    ...
    print(f"Saving Weights for random model")
    ...
    fl.save_checkpoint(modelStateDict, criterionStateDict,
                       ...
                       f"{pathCheckpoint}_random.pt")
    utils.save_logs(logs, pathCheckpoint + "random_logs.json")
    ...
    for epoch in range(startEpoch, nEpoch):
        ...
        if pathCheckpoint is not None \
            ...
            # utils.save_logs(vars(args, pathCheckpoiint + "_args" + logs["epoch"][-1] + ".json")
...
def main(args):
    ...
    # with open(os.path.join(args.pathCheckpoint, 'checkpoint_args.json', 'w')) as json_file:
    ...
    print(' \n   Train path Is: {}'.format(args.pathTrain))
